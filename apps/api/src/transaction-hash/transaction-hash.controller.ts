...
export class TransactionHashController {
    constructor(
        private readonly transactionHashService: TransactionHashService,
    ) {}
    ...
    @ApiResponse({
        status: 201,
        description: 'Transaction hash created successfully',
    })
    @ApiResponse({
        status: 400,
        description: 'Invalid input or rental ID not found',
    })
    ...
    @Get()
    @ApiOperation({
        summary: 'Get all transaction hashes with optional filters',
    })
    @ApiResponse({
        status: 200,
        description: 'Transaction hashes retrieved successfully',
    })
    ...
    @ApiResponse({
        status: 200,
        description: 'Transaction hash deleted successfully',
    })
    @ApiResponse({ status: 404, description: 'Transaction hash not found' })
