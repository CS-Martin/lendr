...
import { CreateBidDto, UpdateBidDto, BidDto, ResponseDto } from '@repo/shared-dtos';
...
export class BidsService implements BidsServiceAbstractClass {
  ...
  async update(rentalPostId: number, updateBidDto: UpdateBidDto): Promise<ResponseDto<BidDto>> {
    this.logger.log('Updating bid', updateBidDto);

    try {
      const bid = await this.bidsDbService.update(rentalPostId, updateBidDto);
  ...
  async findFilteredBids(params: { bidderAddress?: string; rentalPostId?: number }): Promise<ResponseDto<BidDto[]>> {
