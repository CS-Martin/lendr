...
export class TransactionHashService
    implements TransactionHashServiceAbstractClass
{
    ...
    async create(
        ...
        const tx = await this.txDbService.findOne(
            createTransactionHashDto.transactionHash,
        );
        const postId = await this.rentalPostDbService.findOne(
            createTransactionHashDto.rentalId,
        );
        ...
        try {
            const newTx = await this.txDbService.create(
                createTransactionHashDto,
            );
            return {
                statusCode: 201,
    ...
    async find(
        filter?: FilterTransactionHashDto,
    ): Promise<ResponseDto<TransactionHashDto[]>> {
    ...
    async remove(transactionHash: string): Promise<ResponseDto<null>> {
        ...
        try {
            ...
            this.logger.error(
                `Failed to delete transaction hash: ${transactionHash}`,
                error,
            );
