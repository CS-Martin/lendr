...
export class RentalPostsService implements RentalPostsServiceAbstractClass {
    ...
    async findAll(): Promise<ResponseDto<RentalPostDto[]>> {
        ...
        try {
            ...
            const rentalPostDtos = rentalPosts.map((rentalPost) =>
                this.convertToRentalPostDto(rentalPost),
            );
    ...
    async findOne(
        ...
        try {
            const rentalPost =
                await this.rentalPostsDbService.findOne(rentalPostId);

            if (!rentalPost) {
                throw new NotFoundException(
                    `Rental post with ID ${rentalPostId} not found`,
                );
            }
            ...
            this.logger.error(
                `Failed to find rental post with ID ${rentalPostId}`,
                error,
            );
    ...
    async findAllByAddress(
        ...
        try {
            const rentalPosts =
                await this.rentalPostsDbService.findAllByAddress(address);
            ...
            this.logger.error(
                `Failed to find rental posts for address ${address}`,
                error,
            );
            throw new BadRequestException(
                `Failed to find rental posts for address ${address}`,
    ...
    async update(
        ...
        try {
            const rentalPost = await this.rentalPostsDbService.update(
                rentalPostId,
                updateRentalPostDto,
            );
            ...
            this.logger.error(
                `Failed to update rental post with ID ${rentalPostId}`,
                error,
            );
            throw new BadRequestException(
                `Failed to update rental post with ID ${rentalPostId}`,
    ...
    async remove(id: number): Promise<ResponseDto<void>> {
        ...
        try {
            ...
            this.logger.error(
                `Failed to remove rental post with ID ${id}`,
                error,
            );
            throw new BadRequestException(
                `Failed to remove rental post with ID ${id}`,
    ...
    convertToRentalPostDto(rentalPost: RentalPost): RentalPostDto {
        ...
        rentalPostDto.biddingStarttime =
            rentalPost.biddingStarttime ?? undefined;
        rentalPostDto.biddingEndtime = rentalPost.biddingEndtime ?? undefined;
        rentalPostDto.isActive = rentalPost.isActive;
        rentalPostDto.statusCode =
            rentalPost.statusCode as RentalPostStatusEnum;
