...
function BiddingStatus({ rentalPost, bids }: { rentalPost: Doc<"rentalposts">; bids: any[] }) {
  ...
  const highestTotalValue = bids.length > 0
    ? Math.max(...bids.map(bid => bid.bidAmount * bid.rentalDuration))
    : rentalPost.hourlyRate; // Fallback to hourly rate if no bids

  const highestHourlyRate = bids.length > 0
    ...
    <Card className="bg-slate-900/50 border-slate-800">
      <CardContent className="p-6">
        ...
        <div className="mt-4 text-center text-slate-400 text-sm">
          Highest hourly rate: {highestHourlyRate} ETH/hr
        </div>
      </CardContent>
    </Card>
  );
...
function BidItem({ bid, rank, isCurrentUser }: { bid: any; rank: number; isCurrentUser: boolean }) {
  ...
  return (
    <div className={`p-4 rounded-lg border ${isCurrentUser
      ...
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center space-x-2">
          ...
          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${rank === 1
      ...
      <div className="flex justify-between items-center mt-3 text-xs text-slate-500">
        ...
        {bid._updatedTime !== bid._creationTime && (
          <span>Edited</span>
        )}
      </div>
    </div>
...
function CurrentBids({ rentalPost }: { rentalPost: Doc<"rentalposts"> }) {
  ...
  const sortedBids = useMemo(() => {
    return [...bids].sort((a, b) =>
      (b.bidAmount * b.rentalDuration) - (a.bidAmount * a.rentalDuration)
    );
  }, [bids]);
    ...
    return (
      <Card className="bg-slate-900/50 border-slate-800">
        ...
        <CardContent className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="p-4 rounded-lg bg-slate-800/30 border-slate-700/50">
...
export function BiddingTab({ rentalPost }: BiddingTabProps) {
  ...
  return (
    <div className="space-y-6">
      <BiddingStatus rentalPost={rentalPost} bids={allBids} />
