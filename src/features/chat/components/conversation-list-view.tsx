...
export function ConversationListView({ onConversationSelect }: ConversationListViewProps) {
  ...
  const {
    ...
    status,
    loadMore,
  } = usePaginatedQuery(api.conversations.list, user?.address ? { address: user.address } : 'skip', {
    initialNumItems: 10,
  });
                ...
                <div className='flex-1 min-w-0'>
                  <div className='flex items-center justify-between'>
                    <h4 className='font-semibold text-white truncate'>
                      {conversation?.participant?.username || truncateText(conversation?.participant?.address || '')}
                    </h4>
