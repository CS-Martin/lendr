    ...
    args: {
        ...
        const existingBid = await ctx.db
            ...
            .withIndex("by_rentalPost_bidder", (q) =>
                q.eq("rentalPostId", args.rentalPostId)
                    .eq("bidderAddress", args.bidderAddress)
            )
            .first();
...
export const getBidsByRentalPost = query({
    ...
    handler: async (ctx, { rentalPostId, paginationOpts }) => {
        return await ctx.db
            .query("bids")
            .withIndex("by_rentalPost_bidAmount", (q) =>
                q.eq("rentalPostId", rentalPostId)
            )
            .order("desc")
...
export const getUserBidForRentalPost = query({
    ...
    handler: async (ctx, { rentalPostId, bidderAddress }) => {
        return await ctx.db
            ...
            .withIndex("by_rentalPost_bidder", (q) =>
                q.eq("rentalPostId", rentalPostId)
                    .eq("bidderAddress", bidderAddress)
            )
            .first();
    },
});
